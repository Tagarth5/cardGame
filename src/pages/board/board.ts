...
export class BoardPage {
  ...
  constructor(public apiProvider: ApiProvider, public loadingCtrl: LoadingController, public dataProvider: DataProvider, public navCtrl: NavController, public modalCtrl: ModalController) {
    ...
    this.apiProvider.setFactCard().subscribe(()=>{
      this.apiProvider.getFactCards().subscribe((cards)=>{
        this.factCards = cards;
        this.generateScores().subscribe(()=>{     
            console.log(this.factCards); 
            this.apiProvider.setAffirNegCard().subscribe(()=>{
              this.apiProvider.getAffirNegCards().subscribe((cards)=>{
                this.affirNegCards = cards;
                console.log(this.affirNegCards);
                this.assignCards();     
              });
            });               
        });
